...
class LaboratoryDetailScreen extends Component {
  ...
  render() {
    ...
    const insurance =
      details.insurance === "1"
        ? strings('technical_detail.accept_insu')
        : strings('technical_detail.no_accept_insu');
    const subtitle = `${details.owned} - ${details.level} - ${insurance}`;
    return (
      <View style={styles.container}>
        <AppHeader />
        <Toolbar
          title={strings('technical_detail.info')}
        ...
        {!loading && (
          <ScrollView bounces={false} contentContainerStyle={{padding: sizeWidth(2)}}>
            ...
            <View style={styles.bottom}>
              ...
              <Button style={styles.book} text={strings('technical_detail.schedule_test')} onPress={() => this.props.openBookingModal(details.id)}/>
            </View>
