...
class DoctorDetailScreen extends Component {
  ...
  render() {
    ...
    const certificate = this.getCertificate(details);
    return (
      <View style={styles.container}>
        <AppHeader />
        <Toolbar
          title={strings('doctor_detail.info')}
        ...
        {!loading && (
          <ScrollView bounces={false} contentContainerStyle={{padding: sizeWidth(2)}}>
            <DetailSummary
              ...
              subtitle={`${details.shopname} - ${details.facultyName}`}
              summary={details.contentDetail}
              imageUrl={imageUrl}
              title={
                certificate
                  ? `${certificate}. ${details.doctorName}`
                  : `${details.doctorName}`
              }
            />
            <View>
              <Text>{strings('profile.phone')} {details.phone}</Text>
              <Text style={styles.email}>{strings('email')}: {details.email}</Text>
            ...
            <DetailMap style={{marginTop: sizeHeight(1)}}/>
            <Reaction hideDislike={true} numberLike={details.numberlike} userId={this.props.currentUser.customerID} shopId={details.shopid}/>
            <View style={styles.bottom}>
              <Text style={styles.price}>
                <Text>{strings('doctor_detail.service_price')} </Text>
                <Text style={styles.highlight}>
                  {numeral(details.price).format("0,0")} VNĐ
                </Text>
              </Text>
            </View>
            <Button
              style={{marginTop: 10}}
              text={strings('doctor_detail.book_medical')}
              onPress={() => this.props.openBookingModal(details.id)} />
