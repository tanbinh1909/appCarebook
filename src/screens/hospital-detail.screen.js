...
class HospitalDetailScreen extends Component {
  constructor() {
    super();
    // this.state = {
    //   loading: true,
  ...
  shareArticle = async () => {
    try {
      const shareContent = await Api.fetchShareContent(this.state.details.shopid);
      Share.share({
        message: shareContent.shopname + "\n" + shareContent.linkpost
      });
    } catch (err) {}
  };
  ...
  render() {
    ...
    const insurance =
      details.insurance === "1"
        ? strings('hospital_detail.accept_insu')
        : strings('hospital_detail.no_accept_insu');
    const subtitle = `${details.owned} - ${details.level} - ${insurance}`;
    return (
        <View style={styles.container}>
        <AppHeader />
        <Toolbar
          title={strings('hospital_detail.info')}
        ...
        {!loading && (
          <ScrollView bounces={false} contentContainerStyle={{padding: sizeWidth(2)}}>
            ...
            <View style={styles.bottom}>
              ...
              <Button style={styles.book} text={strings('hospital_detail.book_medical')} onPress={() => this.props.openBookingModal(details.id)} />
            </View>
